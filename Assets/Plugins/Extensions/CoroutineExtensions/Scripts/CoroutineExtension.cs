...
namespace SimpleMan.Extensions
{
    public static class ComponentExtension
    {
        ...
        {
            if(time <= 0)
                throw new ArgumentOutOfRangeException("time argument must be greater than 0");
        ...
        {
            if(delay <= 0)
                throw new ArgumentOutOfRangeException("delay argument must be greater than 0");
        ...
        public static IEnumerator WaitUntilCounter(Func<bool> condition, Action callback, float delay = 0)
        {
            while (condition())
            {
                if (delay <= 0)
        ...
        {
            if (frames <= 0)
                throw new ArgumentOutOfRangeException("Frames argument must be greater than 0");
        ...
        public static IEnumerator WaitFramesCounter(int waitFrames, Action callback)
        {
        ...
        public static IEnumerator RepeatUntilCounter(Func<bool> condition, Action repeatAction, Action callback, float delay = 0)
        {
            while (condition())
            {
                repeatAction?.Invoke();
